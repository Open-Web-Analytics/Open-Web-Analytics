...
class Log_observer
{
     ...
     */
    function &factory($class_path, $type, $priority = PEAR_LOG_INFO, $conf = array())
    {
        $type = strtolower($type);
        $class = 'Log_observer_' . $type;

        /* Support both the new-style and old-style file naming conventions. */
        if (file_exists($class_path . '/observer_' . $type . '.php')) {
            $classfile = $class_path. '/observer_' . $type . '.php';
            $newstyle = true;
        } else {
            $classfile = $class_path.'/' . $type . '.php';
            $newstyle = false;
        }
