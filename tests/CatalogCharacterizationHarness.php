...
final class CatalogCharacterizationHarness
{
     ...
     *
     * `parts` is recorded even though `column` is derived from it, because it
     * is not decoration: its PRESENCE is what makes lookupDimension()
     * substitute the table alias into the SQL rather than write it in front.
     * An expression registered without it would record identically to one
     * registered with it and produce `event.CONCAT(...)` at query time, so
     * leaving it out would mean the recording could not see the difference
     * between a dimension that works and one that cannot run.
     */
    private const DIMENSION_KEYS = array(
        'name', 'entity', 'column', 'family', 'label', 'description',
        'foreign_key_name', 'data_type', 'denormalized', 'parts',
    );
