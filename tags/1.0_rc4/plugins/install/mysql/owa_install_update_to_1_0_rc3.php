...
class owa_install_update_to_1_0_rc3 extends owa_install {
	 ...
	 * @var string
	 */
	var $package_display_name = 'OWA Update to 1.0RC3 for MySQL';
	 ...
	 * @var string
	 */
	var $description = 'This is the update to 1.0RC3 for MySQL 4 or greater.';
	...
	function create_visitors_table() {
		...
		return $this->db->query(
			sprintf("
			ALTER TABLE %1\$s ADD COLUMN first_session_timestamp BIGINT
			",
	...
	function create_impressions_table() {
		...
		$this->db->query(
			sprintf("DROP TABLE %s", $this->config['ns'].$this->config['impressions_table']));
