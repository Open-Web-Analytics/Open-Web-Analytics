<?php foreach($visits as $visit): ?>
		...
		<div>
			...
			<table>
				<TR>
					...
					<TD>	
						<div class="visitor_info_box pages_box">
							<span class="large_number"><a href="<?=OWA_REPORTING_URL;?>/session_report.php&wa_s=<?=$visit['session_id']?>"><?=$visit['num_pageviews'];?></span><br /><span class="info_text">Pages</span></a>
						</div>
					...
					<TD>
					<span class="h_label">Visitor:</span> <a href="<?=OWA_REPORTING_URL;?>/visitor_report.php&wa_v=<?=$visit['visitor_id'] ?>"><span class="inline_h2"><? if (!empty($visit['user_name'])):?><?=$visit['user_name'];?><?elseif (!empty($visit['user_email'])):?><?=$visit['user_email'];?><? else: ?><?=$visit['visitor_id'];?><? endif; ?></span></a> via <span class="info_text"><?=$visit['host'];?></span> located in <span class="info_text"><?=$visit['city'];?>, <?=$visit['country'];?></span>
				...
				| <a href="<?=OWA_REPORTING_URL;?>/session_report.php&wa_s=<?=$visit['prior_session_id']?>">Last visit</a> was <?=round($visit['time_sinse_priorsession']/(3600*24));?> 
