...
class Module extends \OWA\Core\Module {
    ...
    protected function registerCubeDimensions() {
        ...
        foreach ( self::cubeColumnDimensions() as $name => $d ) {

            $this->registerDimension(
                $name, $cube, $d['column'], $d['label'], $d['family'],
                $d['description'], '', true,
    ...
    /**
     * The dimensions that are a cube column and nothing more.
     *
     * A table rather than fifty calls, because that is what they are: a name, a
     * column and what to call it. The computed kinds PLAN 2.4.1 describes --
     * concat, map, datePart, test, filtered -- are not here; each needs a
     * renderer, and none of them is a column read.
     *
     * EVERY ONE IS DENORMALISED, structurally: the cube carries its values as
     * columns on the event row, so there is no dimension table to join and no
     * foreign key to name.
     *
     ...
     * (PLAN 2.23).
     *
     * @return array name => [column, label, family, description, data_type?]
     */
    public static function cubeColumnDimensions() {
