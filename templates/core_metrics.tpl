...
<table width="100%">
	...
	<? if ($period == 'this_year' || $params['period'] == 'this_year'): ?>
		<TD>
		<a href="<?=$this->make_report_link('dashdate_report.php', array('period' => 'month', 'year' => $row['year'], 'month' => $row['month']));?>">
	...
	<? else: ?>
		...
		<TD>
		<a href="<?=$this->make_report_link('dashdate_report.php', array('period' => 'day', 'year' => $row['year'], 'month' => $row['month'], 'day' => $row['day']));?>">
		<?=$row['day'];?>
		</a>
		</TD>
		<TD><?=$row['year'];?></TD>
