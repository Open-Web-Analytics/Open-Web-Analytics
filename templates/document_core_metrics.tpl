...
<table width="100%">
	...
	<? if ($period == 'this_year' || $params['period'] == 'this_year'): ?>
		<TD>
		<a href="<?=$this->make_report_link('document_report.php', array('year' => $row['year'], 'month' => $row['month'], 'document_id' => $params['document_id']));?>">
	...
	<? else: ?>
		...
		<TD>
		<a href="<?=$this->make_report_link('document_report.php', array('year' => $row['year'], 'month' => $row['month'], 'day' => $row['day'], 'document_id' => $params['document_id']));?>">
		<?=$row['day'];?>
		</a>
		</TD>
		<TD><?=$row['year'];?></TD>
